...
# gmail_utils.py - modificar la función enviar_email_html_con_adjuntos
...
def calcular_sumatorias_csv(ruta_csv):
    ...
    try:
        ...
        with open(ruta_csv, 'r', encoding='utf-8') as f:
            ...
            for fila in reader:
                ...
                try:
                    ...
                    # DEBUG: Mostrar valores de las primeras filas
                    if fila_numero <= 5:
                        print(f"\n🔍 DEBUG CSV - Fila {fila_numero}:")
                        print(f"  Col I (aporte personal): {aporte_personal:.2f}")
                        print(f"  Col Q (reajuste aporte pers): {reaj_aporte_pers:.2f}")
                        print(f"  → Total Personal fila: {(aporte_personal + reaj_aporte_pers):.2f}")
                        ...
                        print(f"  → Total Adherente fila: {(adherente_sec + hijo_menor_35 + menor_cargo + reaj_adherente_sec + reajuste_hijo_menor + reajuste_menor_cargo):.2f}")
                    ...
                    if fila_numero <= 5:
                        print(f"  → Sumas acumuladas hasta fila {fila_numero}:")
                        print(f"     Personal: {sumatorias['personal']:.2f}")
                        print(f"     Adherente: {sumatorias['adherente']:.2f}")
                        print(f"     Fondo V: {sumatorias['fondo_voluntario']:.2f}")
                        print(f"     Créditos: {sumatorias['creditos_asistenciales']:.2f}")
        ...
        print(f"\n📊 TOTALES CALCULADOS DESDE CSV (CÁLCULOS CORREGIDOS):")
        print(f"  Personal (I+Q): {sumatorias['personal']:,.2f}")
        print(f"  Adherente (J+L+M+R+T+U): {sumatorias['adherente']:,.2f}")
        print(f"  Fondo Voluntario (K+S): {sumatorias['fondo_voluntario']:,.2f}")
        print(f"  Créditos Asistenciales (N+V): {sumatorias['creditos_asistenciales']:,.2f}")
        print(f"  Patronal (W+X): {sumatorias['patronal']:,.2f}")
        print(f"  TOTAL GENERAL: {sumatorias['total']:,.2f}")
