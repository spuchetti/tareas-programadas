...
#   3 -> falló el sync por otro motivo NO técnico (ej. --max-delete, un
#        ID de carpeta de origen mal configurado) -- no es login/red, así
#        que no hay que mandar a revisar conectividad con Mega
# En los cuatro casos el caller sigue con las demás carpetas en vez de
...
sync_uno() {
  ...
  # A diferencia de lo que se asumía antes, un fallo de sync SÍ puede no
  # ser un error técnico de login/red: --max-delete cortando el sync
  # porque el origen tiene muchos menos archivos que el destino (ej. un
  # ID de carpeta de Drive mal configurado apuntando a un año viejo) sale
  # con rc_sync=1 igual que cualquier otro "otro fallo de rclone" que no
  # matchea PATRON_ERROR_TECNICO_RCLONE. Reportarlo como "error técnico:
  # revisar conectividad/credenciales de Mega" es engañoso y manda a
  # buscar en el lugar equivocado (pasó en un run real: la causa fue un
  # FOLDER_REPARTICIONES_ID apuntando a la carpeta de 2025 por error).
  if [[ "$rc_sync" -eq 2 ]]; then
    echo "❌ El sync de '${destino}' falló por error técnico (login/red) tras agotar reintentos."
    return 2
  elif [[ "$rc_sync" -ne 0 ]]; then
    echo "❌ El sync de '${destino}' falló (código ${rc_sync}, no es un error técnico de login/red)."
    echo "❌ Revisar el log de arriba: puede ser --max-delete, un ID de carpeta de origen mal"
    echo "❌ configurado, u otro motivo de contenido -- NO conectividad/credenciales de Mega."
    return 3
  fi
...
if [[ "$HUBO_FALLO_NO_TECNICO" -eq 1 ]]; then
  echo "⚠️  Backup diario con FALLOS DE SYNC NO TÉCNICOS ($(date))"
  echo "⚠️  Al menos un destino falló por un motivo que NO es login/red ni cambios sospechosos"
  echo "⚠️  (ej. --max-delete cortando el sync, un ID de carpeta de origen mal configurado)."
  echo "⚠️  Revisar el log de ese destino arriba -- NO es un problema de conectividad con Mega."
fi
if [[ "$HUBO_ALERTA_RANSOMWARE" -eq 0 && "$HUBO_ERROR_TECNICO" -eq 0 && "$HUBO_FALLO_NO_TECNICO" -eq 0 ]]; then
...
if [[ "$HUBO_ALERTA_RANSOMWARE" -eq 1 ]]; then
  ...
  exit 2
elif [[ "$HUBO_FALLO_NO_TECNICO" -eq 1 ]]; then
  exit 3
else
  exit 0
fi